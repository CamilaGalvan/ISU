Public Class frmISU
    ...
    Sub Permisos(ByVal Usuario As String)
        'If CN.State <> 1 Then
        'MsgBox("La conexión con la base de datos esta abierta")
        'Else
        '   Sql = "Select rol from usuario_rol where usuario ='" & Usuario & "'"
        '   Try
        '       rs.Open(Sql, CN)
        '   Catch ex As Exception
        '       MsgBox("Error al obtener rol de usuario")
        '       Exit Sub
        '   End Try
        '   If rs.RecordCount() = 0 Then
        '       MsgBox("El rol de usuario no esta cargado")
        '   Else
    ...
    Private Sub btnInicio_Click(sender As System.Object, e As System.EventArgs) Handles btnInicio.Click
        'Try
        'CN.Open("MIODBC", txtUsuario.Text, txtContraseña.Text)
        'coenxion=1
        ' Catch ex As Exception
    ...
    Private Sub cbxIdioma_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cbxIdioma.SelectedIndexChanged
        ...
        tsTambo.Enabled = tambo
        tsPersonal.Enabled = personal
        tsAnimal.Enabled = animal
        tsSesion.Enabled = sesion
        tsOrdeñe.Enabled = ordeñe
        tsEnlistar.Enabled = enlistar
        tsVenta.Enabled = venta
    End Sub
